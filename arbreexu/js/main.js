...
GEOR.Addons.arbreexu.prototype = {
	...
	onExecuted: function(resp) {
		...
		for (var keyWmsOutputs = 1; keyWmsOutputs <= arbreexu.outputs.wms.list.length; keyWmsOutputs++) {
			// wmsValue : http://geoxxx.agrocampus-ouest.fr:80/geoserverwps/wfs?+++cseb:vue_d_ensemble2 
			arbreexu.outputs.wms['wms'+keyWmsOutputs].name = arbreexu.outputs.wms['wms'+keyWmsOutputs].wmsValue.substring(arbreexu.outputs.wms['wms'+keyWmsOutputs].wmsValue.indexOf('?') + 1); // cseb:vue_d_ensemble2 
			arbreexu.outputs.wms['wms'+keyWmsOutputs].url = arbreexu.outputs.wms['wms'+keyWmsOutputs].wmsValue.substr(0, arbreexu.outputs.wms['wms'+keyWmsOutputs].wmsValue.indexOf('?'));
			console.log("Une couche WMS a été ajoutée :");
			console.log("	- URL : " + arbreexu.outputs.wms['wms'+keyWmsOutputs].url);
			console.log("	- Nom : " + arbreexu.outputs.wms['wms'+keyWmsOutputs].name); 		
			
		// PART 2 : Ajout du WMS	
			arbreexu.outputs.wms['wms'+keyWmsOutputs].layer = new OpenLayers.Layer.WMS("wms"+keyWmsOutputs,
					arbreexu.outputs.wms['wms'+keyWmsOutputs].url, 
					{'layers': arbreexu.outputs.wms['wms'+keyWmsOutputs].name,transparent: true} //, transparent: true, format: 'image/gif'
					//,{isBaseLayer: true}
				);
			
			arbreexu.outputs.wms['wms'+keyWmsOutputs].cReccord = GEOR.util.createRecordType();
			arbreexu.outputs.wms['wms'+keyWmsOutputs].lReccord = new arbreexu.outputs.wms['wms'+keyWmsOutputs].cReccord({
				layer: arbreexu.outputs.wms['wms'+keyWmsOutputs].layer,
				name: arbreexu.outputs.wms['wms'+keyWmsOutputs].name, 
				type: "WMS"
			});
			arbreexu.outputs.wms['wms'+keyWmsOutputs].clone = arbreexu.outputs.wms['wms'+keyWmsOutputs].lReccord.clone();
			console.log(keyWmsOutputs);
			GEOR.ows.hydrateLayerRecord(arbreexu.outputs.wms['wms'+keyWmsOutputs].clone, { // flag j'arrive pas a lui donner du i
				success: function(index) {
					console.log(index);
					console.log(arbreexu.outputs.wms['wms'+index].clone.get("title"));
					arbreexu.outputs.wms['wms'+index].clone.get("layer").setName(arbreexu.outputs.wms['wms'+index].clone.get("title")); // flag no title
					layerStore.addSorted(arbreexu.outputs.wms['wms'+index].clone);
